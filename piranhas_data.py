...
def gera_grafico_temperaturas(dados, estatisticas_temp):
    ...
    plt.title('Temperaturas Máxima e Mínima em Piranhas')
    plt.xlabel('Data')
    ...
    plt.figtext(0.018, 0.03, f'Modas das temperaturas máximas: {estatisticas_temp["moda_temp_max_value"]}°C', color='red', fontsize=10, ha='left')
    plt.figtext(0.25, 0.01, f'T. Mínima mais baixa: {estatisticas_temp["temperatura_menor"]}°C', color='blue', fontsize=10, ha='left')
    plt.figtext(0.25, 0.03, f'Moda das temperaturas minimas: {estatisticas_temp["moda_temp_min_value"]}°C', color='blue', fontsize=10, ha='left')
    plt.figtext(0.55, 0.03, f'Média da Temperatura Mínima: {estatisticas_temp["media_temp_min"]}°C', color='blue', fontsize=10, ha='left')
...
def calcular_estatisticas_precipitacao(dados):
    ...
    if not moda_precipitacao.empty:
        moda_precipitacao_value = round(moda_precipitacao.iloc[0], 2)
    else:
...
def gerar_grafico_precipitacao(dados, estatisticas_precipitacao):
    ...
    plt.title('Precipitação Total Diária de Piranhas')
    plt.xlabel('Data')
    ...
    if not dados_filtrados.empty:
        ...
        plt.figtext(0.50, 0.01, f'Não houve precipitações na base de dados', color='black', fontsize=10, ha='left')
    plt.tight_layout()
    plt.show()
